...
select * from customer_nodes;
select region_id,count(node_id) as no_of_nodes from customer_nodes group by region_id order by region_id asc;

#How many customers are allocated to each region?
select * from customer_nodes;
select region_id ,count(customer_id) as no_of_customers from customer_nodes group by region_id order by ;

#How many days on average are customers reallocated to a different node?
select * from customer_nodes;
select * from regions;
select avg(datediff(start_date,end_date)) as avg_customers_reallocated from customer_nodes;

#What is the median, 80th and 95th percentile for this same reallocation days metric for each region?
select * from
SELECT 
  r.region_name,
  PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY DATEDIFF(c.end_date, c.start_date)) AS median_days,
  PERCENTILE_CONT(0.80) WITHIN GROUP (ORDER BY DATEDIFF(c.end_date, c.start_date)) AS p80_days,
  PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY DATEDIFF(c.end_date, c.start_date)) AS p95_days
FROM 
  customer_allocations c
JOIN 
...
select txn_type ,count(distinct(customer_id)) as unique_ids, sum(txn_amount) as total_amt from customer_transactions group by txn_type;

#What is the average total historical deposit counts and amounts for all customers?
select count(txn_type) as avg_deposit , sum(txn_amount) from customer_transactions where txn_type = 'deposit';
SELECT 
  AVG(deposit_count) AS avg_deposit_count_per_customer,
  AVG(deposit_amount) AS avg_deposit_amount_per_customer
FROM (
  SELECT 
    customer_id,
...
increased_balance AS (
  ...
  WHERE previous_balance IS NOT NULL
    AND ((closing_balance - previous_balance) / previous_balance) > 0.05
)
SELECT 
  ROUND((COUNT(DISTINCT ib.customer_id) / COUNT(DISTINCT mb.customer_id)) * 100, 2) AS percentage_customers_increased
FROM 
  increased_balance ib,
  monthly_balances mb;
